					...
					<CardActionArea>
						<Link to={`/comics/${comic.id}`}>
							<CardMedia
								...
								component='img'
								image={comic.thumbnail && comic.thumbnail.path ? comic.thumbnail.path + '.'+ comic.thumbnail.extension : noImage}
								title='show image'
							/>
        ...
        if (error){
            ...
            return (
                <div>
                    ...
                    <br />
                    {(currentPage === 0) ? "": 
                <Link className="showlink" to={`/comics/page/${Number(currentPage )- 1}`} onClick={onClickPrev}>
                  Previous
                </Link>}
          
            {(nextPage !== true) ? "" : 
                <Link className="showlink" to={`/comics/page/${Number(currentPage) + 1}`} onClick={onClickNext}>
                  Next
                </Link>}
                     <br></br>
     <br></br>
